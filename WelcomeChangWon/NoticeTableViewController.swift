...
class NoticeTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if self.notice.count == 0 {
            ref?.child("Community").child("Notice").observe(.childAdded, with: { (snapshot) in
                if snapshot.value is NSNull {
                    print("없다")
                } else {
                    if let item = snapshot.value as? [String : String] {
                        self.notice.append(item)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: .subtitle, reuseIdentifier: "notice")
        let dic = notice[indexPath.row]
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let destination = segue.destination as? noticeViewController {
            destination.labelcontent = notice[index.row]["detailcontent"]!
        }
    }
