...
class FestivalViewController: UIViewController, MTMapViewDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewWillAppear(_ animated: Bool) {
        self.handle = self.ref?.child("Review").child(self.Location).child("축제").child(self.List["축제명"]!).observe(.childAdded, with: { (snapshot) in
    ...
    @IBAction func previous(_ sender: UIBarButtonItem) {
        handle = ref?.child(Location).child("축제").observe(.childAdded, with: { (snapshot) in
            if let item = snapshot.value as? [String : String] {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        items.append(poiItem(name: List["축제명"]!, latitude: Double(List["위도"]!)!, longitude: Double(List["경도"]!)!))
    ...
    override func viewDidLoad() {
        ...
        name.text = "이름 : \(List["축제명"]!)"
        address.text = "주소 : \(List["소재지도로명주소"]!)"
        intro.text = "설명 : \(List["축제내용"]!)"
    ...
    @objc func AddCourse() { //코스 등록 버튼 액션 눌리면
        let dic = ["name": List["축제명"]!, "address": List["소재지도로명주소"]!, "image": List["사진"]!, "lat": List["위도"]!, "lon": List["경도"]!, "intro": List["축제내용"]!]
    ...
    @objc func Thumb() { //좋아요 버튼을 눌렀을 시
        if List["Count"] == nil { // 카운트라는 값이 없을 시 즉, 아무런 카운트가 없을 시
            let dic = ["Count" : "1"]
            ref?.child(Location).child("축제").child(List["축제명"]!).updateChildValues(dic)
            List["Count"] = "1"
            Favorite.isEnabled = false
            return
        } else { // 하나라도 값이 있을 시
            List["Count"] = String(Int(List["Count"]!)! + 1)
            ref?.child(Location).child("축제").child(List["축제명"]!).updateChildValues(List)
            Favorite.isEnabled = false
            return
        }
    }
    @objc func ActReview() {
        ...
        let confirm = UIAlertAction(title: "작성", style: .default) { //댓글을 작성하고 확인을 누르면 해당 관광지 란에 댓글 데이터 입력
            ...
            self.ref?.child("Review").child(self.Location).child("축제").child(self.List["축제명"]!).childByAutoId().setValue(dic)
            if self.List["ReviewCount"] == nil { // 카운트라는 값이 없을 시 즉, 아무런 카운트가 없을 시
                let dic = ["ReviewCount" : "1"]
                self.ref?.child(self.Location).child("축제").child(self.List["축제명"]!).updateChildValues(dic)
                self.List["ReviewCount"] = "1"
                //self.Favorite.isEnabled = false
            } else { // 하나라도 값이 있을 시
                self.List["ReviewCount"] = String(Int(self.List["ReviewCount"]!)! + 1)
                self.ref?.child(self.Location).child("축제").child(self.List["축제명"]!).updateChildValues(self.List)
                //self.Favorite.isEnabled = false
            }
        }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete { // 댓글 삭제 시
            ...
            if dic["name"] == Auth.auth().currentUser?.email {// 삭제 클릭시 네임과 실제 접속 유저의 이메일과 동일하면 즉 자기가 쓴 글을 자기가 접근을 했을 시
                ref?.child("Review").child(self.Location).child("축제").child(self.List["축제명"]!).observe(.childAdded, with: { (snapshot) in
                    if snapshot.value is NSNull {
                        print("Nothing")
                    } else {
                        if let item = snapshot.value as? [String:String] {
                            if dic["Content"] == item["Content"] {
                                self.ref?.child("Review").child(self.Location).child("축제").child(self.List["축제명"]!).child(snapshot.key).removeValue()
                                self.reviewList.remove(at: indexPath.row)
                                self.List["ReviewCount"] = String(Int(self.List["ReviewCount"]!)!-1)
                                self.ref?.child(self.Location).child("축제").child(self.List["축제명"]!).setValue(self.List)
