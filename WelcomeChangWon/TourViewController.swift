...
class TourViewController: UIViewController, MTMapViewDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var homepageBut : UIButton!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if reviewList.count == 0 {
            self.handle = self.ref?.child("Review").child(self.Location).child("관광지").child(self.List["관광안내소명"]!).observe(.childAdded, with: { (snapshot) in
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        items.append(poiItem(name: List["관광안내소명"]!, latitude: Double(List["위도"]!)!, longitude: Double(List["경도"]!)!))
    ...
    @IBAction func Previous(_ sender: UIBarButtonItem) {
        handle = ref?.child(Location).child("관광지").observe(.childAdded, with: { (snapshot) in
            if let item = snapshot.value as? [String : String] {
    ...
    @objc func Thumb() { //좋아요 버튼을 눌렀을 시
        if List["Count"] == nil { // 카운트라는 값이 없을 시 즉, 아무런 카운트가 없을 시
            let dic = ["Count" : "1"]
            ref?.child(Location).child("관광지").child(List["관광안내소명"]!).updateChildValues(dic)
            List["Count"] = "1"
            Favorite.isEnabled = false
        } else { // 하나라도 값이 있을 시

            List["Count"] = String(Int(List["Count"]!)! + 1)
            ref?.child(Location).child("관광지").child(List["관광안내소명"]!).updateChildValues(List)
            Favorite.isEnabled = false
        }
    }
    override func viewDidLoad() {
        ...
        imageBtn1.image = UIImage(named: List["사진"]!)
        imageBtn2.image = UIImage(named: List["사진1"]!)
        imageBtn3.image = UIImage(named: List["사진2"]!)
        callBut.addTarget(self, action: #selector(ActCall(sender:)), for: .touchUpInside)
        homepageBut.addTarget(self, action: #selector(ActHomePage), for: .touchUpInside)
        ...
        name.text = "이름 : \(List["관광안내소명"]!)"
        address.text = "주소 : \(List["소재지도로명주소"]!)"
        intro.text = "설명 : \(List["안내소소개"]!)"
        ...
        Course.isHighlighted = true
        // Do any additional setup after loading the view.
    }
    @objc func AddCourse() { //코스 등록 버튼 액션 눌리면
        let dic = ["name": List["관광안내소명"]!, "address": List["소재지도로명주소"]!, "image": List["사진"]!, "lat": List["위도"]!, "lon": List["경도"]!, "intro": List["안내소소개"]!]
    ...
    @objc func ActReview() {
        ...
        let confirm = UIAlertAction(title: "작성", style: .default) { //댓글을 작성하고 확인을 누르면 해당 관광지 란에 댓글 데이터 입력
            ...
            self.ref?.child("Review").child(self.Location).child("관광지").child(self.List["관광안내소명"]!).childByAutoId().setValue(dic)
            if self.List["ReviewCount"] == nil { // 카운트라는 값이 없을 시 즉, 아무런 카운트가 없을 시
                let dic = ["ReviewCount" : "1"]
                self.ref?.child(self.Location).child("관광지").child(self.List["관광안내소명"]!).updateChildValues(dic)
                self.List["ReviewCount"] = "1"
                //self.Favorite.isEnabled = false
            } else { // 하나라도 값이 있을 시
                print("하나 찍히네?")
                self.List["ReviewCount"] = String(Int(self.List["ReviewCount"]!)! + 1)
                self.ref?.child(self.Location).child("관광지").child(self.List["관광안내소명"]!).updateChildValues(self.List)
                //self.Favorite.isEnabled = false
            }
        }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete { // 댓글 삭제 시
            ...
            if dic["name"] == Auth.auth().currentUser?.email {// 삭제 클릭시 네임과 실제 접속 유저의 이메일과 동일하면 즉 자기가 쓴 글을 자기가 접근을 했을 시
                ref?.child("Review").child(self.Location).child("관광지").child(self.List["관광안내소명"]!).observe(.childAdded, with: { (snapshot) in
                    if snapshot.value is NSNull {
                        print("Nothing")
                    } else {
                        if let item = snapshot.value as? [String:String] {
                            if dic["Content"] == item["Content"] {
                                self.ref?.child("Review").child(self.Location).child("관광지").child(self.List["관광안내소명"]!).child(snapshot.key).removeValue()
                                self.reviewList.remove(at: indexPath.row)
                                self.List["ReviewCount"] = String(Int(self.List["ReviewCount"]!)!-1)
                                self.ref?.child(self.Location).child("관광지").child(self.List["관광안내소명"]!).setValue(self.List)
    ...
    @objc func ActCall(sender : UIButton) {
        if let url = URL(string: "telprompt://\(List["안내소전화번호"]!)") {
            UIApplication.shared.open(url, options: [:], completionHandler: nil)
        }
    }
    @objc func ActHomePage(sender : UIButton) {
        let storyBoard = UIStoryboard(name: "Main", bundle: nil)
        if let webViewController = storyBoard.instantiateViewController(withIdentifier: "HomePage") as? HomePageViewController {
            print("찍히는거같은디?")
            webViewController.address = List["홈페이지주소"]!
